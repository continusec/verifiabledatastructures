...
func TestBPathLen(t *testing.T) {
	if BPath(nil).Length() != 0 {
		t.FailNow()
	}
	if BPath([]byte{0}).Length() != 256 {
		t.FailNow()
	}
	if BPath([]byte{1}).Length() != 1 {
		t.FailNow()
	}
	if BPath([]byte{255}).Length() != 255 {
		t.FailNow()
	}
}

func TestBPathSlicing(t *testing.T) {
	foo := BPathFromKey([]byte("foo"))
...
func TestBPathJoining(t *testing.T) {
	...
	bPathMatches(t, BPathJoin(foo.Slice(30, 30), BPathTrue), []bool{true})
	bPathMatches(t, BPathJoin(foo.Slice(1, 5), foo.Slice(2, 4)), []bool{false, true, false, true, true, false})
}

func bPathMatches(t *testing.T, b BPath, expected []bool) {
