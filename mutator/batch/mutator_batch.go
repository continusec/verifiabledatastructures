...
func (bm *Mutator) Create() (verifiable.MutatorService, error) {
	rv := &batchMutatorImpl{
		Conf: bm,
...
func (bm *Mutator) MustCreate() verifiable.MutatorService {
	rv, err := bm.Create()
	if err != nil {
		log.Fatal(err)
	}
...
func (bm *batchMutatorImpl) handleBatch(ctx context.Context, kw verifiable.KeyWriter, startSize int64, seed *chObject) (int64, *chObject, error) {
	curSize := startSize
	var err error
	obj := seed
	var ok bool
	...
	for {
		...
		curSize, err = verifiable.ApplyMutation(ctx, kw, curSize, obj.mut)
		if err != nil {
			return 0, nil, err
		}
		cnt--
...
func (bm *batchMutatorImpl) consume() {
	...
	for {
		...
		ns := seed.ns
		err := bm.Conf.Writer.ExecuteReadOnly(ctx, ns, func(ctx context.Context, kr verifiable.KeyReader) error {
			wrapper.Parent = kr

			startSize, err = verifiable.ReadObjectSize(ctx, kr)
			if err != nil {
				return err
			}
		...
		if nextSize > startSize { // save it out
			err = bm.Conf.Writer.ExecuteUpdate(ctx, ns, func(ctx context.Context, kw verifiable.KeyWriter) error {
				...
				return verifiable.WriteObjectSize(ctx, kw, nextSize)
			})
			if err != nil {
				log.Fatal(err)
			}
