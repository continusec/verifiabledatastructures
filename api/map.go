...
func (l *serverMap) verifyAccessForPermission(perm pb.Permission) error {
...
func (m *serverMap) MutationLog() client.VerifiableLog {
	return &serverLog{
		account: m.account,
...
func (m *serverMap) TreeHeadLog() client.VerifiableLog {
	return &serverLog{
		account: m.account,
...
func (m *serverMap) Create() error {
	err := m.verifyAccessForPermission(pb.Permission_PERM_MAP_CREATE)
	if err != nil {
		return err
	}
	if len(m.name) == 0 {
...
func (m *serverMap) Destroy() error {
	err := m.verifyAccessForPermission(pb.Permission_PERM_MAP_DELETE)
	if err != nil {
		return err
	}
	promise, err := m.account.Service.Mutator.QueueMutation(&pb.Mutation{
...
// Get will return the value for the given key at the given treeSize. Pass client.Head
...
func (m *serverMap) Get(key []byte, treeSize int64, factory client.VerifiableEntryFactory) (*client.MapInclusionProof, error) {
	err := m.verifyAccessForPermission(pb.Permission_PERM_MAP_GET_VALUE)
	if err != nil {
		return nil, err
	}
...
func (m *serverMap) Set(key []byte, value client.UploadableEntry) (*client.AddEntryResponse, error) {
	err := m.verifyAccessForPermission(pb.Permission_PERM_MAP_SET_VALUE)
	if err != nil {
		return nil, err
	}
...
// While this will return quickly, the change will be reflected asynchronously in the map.
// Returns an AddEntryResponse which contains the leaf hash for the mutation log entry.
func (m *serverMap) Update(key []byte, value client.UploadableEntry, previousLeaf client.MerkleTreeLeaf) (*client.AddEntryResponse, error) {
	err := m.verifyAccessForPermission(pb.Permission_PERM_MAP_SET_VALUE)
	if err != nil {
		return nil, err
	}
...
// While this will return quickly, the change will be reflected asynchronously in the map.
// Returns an AddEntryResponse which contains the leaf hash for the mutation log entry.
func (m *serverMap) Delete(key []byte) (*client.AddEntryResponse, error) {
	err := m.verifyAccessForPermission(pb.Permission_PERM_MAP_SET_VALUE)
	if err != nil {
		return nil, err
	}
...
// TreeHead returns map root hash for the map at the given tree size. Specify client.Head
// to receive a root hash for the latest tree size.
func (m *serverMap) TreeHead(treeSize int64) (*client.MapTreeHead, error) {
	err := m.verifyAccessForPermission(pb.Permission_PERM_MAP_GET_VALUE)
	if err != nil {
		return nil, err
	}
