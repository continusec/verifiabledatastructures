...
func (m *serverMap) MutationLog() continusec.VerifiableLog {
	return &serverLog{
		account: m.account,
...
func (m *serverMap) TreeHeadLog() continusec.VerifiableLog {
	return &serverLog{
		account: m.account,
...
func (m *serverMap) Get(key []byte, treeSize int64, factory continusec.VerifiableEntryFactory) (*continusec.MapInclusionProof, error) {
...
func (m *serverMap) Set(key []byte, value continusec.UploadableEntry) (*continusec.AddEntryResponse, error) {
...
// While this will return quickly, the change will be reflected asynchronously in the map.
// Returns an AddEntryResponse which contains the leaf hash for the mutation log entry.
func (m *serverMap) Update(key []byte, value continusec.UploadableEntry, previousLeaf continusec.MerkleTreeLeaf) (*continusec.AddEntryResponse, error) {
	err := m.verifyAccessForPermission(pb.Permission_PERM_MAP_SET_VALUE)
	if err != nil {
		return nil, err
	}
...
// While this will return quickly, the change will be reflected asynchronously in the map.
// Returns an AddEntryResponse which contains the leaf hash for the mutation log entry.
func (m *serverMap) Delete(key []byte) (*continusec.AddEntryResponse, error) {
	err := m.verifyAccessForPermission(pb.Permission_PERM_MAP_SET_VALUE)
	if err != nil {
		return nil, err
	}
...
func (m *serverMap) TreeHead(treeSize int64) (*continusec.MapTreeHead, error) {
	err := m.verifyAccessForPermission(pb.Permission_PERM_MAP_GET_VALUE)
	if err != nil {
		return nil, err
	}
